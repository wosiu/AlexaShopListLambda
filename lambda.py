...
def get_welcome_response():
    ...
    session_attributes = {}
    card_title = "Welcome"
    speech_output = "What would you like to put to your online bucket"
    # If the user either does not reply to the welcome message or says something
    # that is not understood, they will be prompted again with this text.
    reprompt_text = "Please tell me what you would like add to your shopping list by saying for example, " \
                    "add red wine."
...
def handle_session_end_request():
    ...
    speech_output = "Thank you for using Alexa online bucket. You can check online comparison of your products." \
                    "Have a nice day! "
...
def add_product(intent, session):
    ...
    if 'Product' in intent['slots'] and intent['slots']['Product']['value'] is not None:
        ...
        path = '/resolve/%s'
        url = host + path % urllib2.quote(product_name)
        print("Connecting to " + url)
        resp = urllib2.urlopen(url).read()
        parsed = json.loads(resp)
        print("Response status" + parsed['status'])
        
        if not 'status' in parsed:
            reprompt_text = "You din't paid a bill. Server is turned off."
            speech_output = reprompt_text            
        elif parsed['status'] == 'OK':
            reprompt_text = "I've just added " + product_name + "to your shop list"
            speech_output = "Sure! " + product_name + " added"
        else:
            msg = parsed['msg']
            speech_output = "I have a problem with adding " + product_name
            if msg:
                speech_output = speech_output + ". " + msg
            reprompt_text = "Sorry, problem occured. Try one more time."
    else:
